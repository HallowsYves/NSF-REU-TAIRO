"""
Evaluate Recovery v4-HX5 (fast-attack/slow-release trigger EMA, see
recovery/recovery_v4_hx5.py) against v4, v4_hx2, and sac_her, plus the
specific goal-spoofing comparison against sac_her_recovery_v3 that motivated
this fix in the first place.

Same pattern as scripts/evaluate_recovery_v4_hx4.py: hx5 is the only newly
collected method (results/data_recovery_v4_hx5/, n=450, seeds 0-14, all 11
conditions); sac_her/v4/v4_hx2 baselines are reused from existing sources
already collected at the same power level. v3 is only available at the older
n=150 (seeds 0-4) -- that comparison is reported separately, not pooled with
the n=450 comparisons. No new statistical code -- reuses
compare_two_methods/bh_adjust from scripts/audit_recovery_do_no_harm.py.

Output: results/recovery_v4_hx5_evaluation.csv
"""
...
DATA_SOURCES = _BASE_DATA_SOURCES + ["results/data_recovery_v4_hx5"]
...
def main():
    ...
    print("\n[eval] Seed coverage per method x condition (hx5 row should show 15 everywhere):")
    print(coverage.to_string())

    # -- hx5 vs v4 and hx5 vs hx2, full 11-condition grid (n=450, paired) -----
    rows = []
    for baseline in ["sac_her_recovery_v4", "sac_her_recovery_v4_hx2"]:
        for condition in ALL_CONDITIONS:
            row = compare_two_methods(combined, "sac_her_recovery_v4_hx5", baseline, condition)
            if row is not None:
                rows.append(row)
    ...
    print("\n[eval] hx5 vs v4 / hx5 vs hx2, goal-spoof conditions (the targeted fix):")
    print(vs_df[vs_df.condition.isin(GOAL_SPOOF_CONDITIONS)].to_string(index=False))

    print("\n[eval] hx5 vs v4 / hx5 vs hx2, grip_state_falsification (must confirm hx2's win survives):")
    print(vs_df[vs_df.condition == "grip_state_falsification"].to_string(index=False))

    print("\n[eval] All hx5-vs-v4 / hx5-vs-hx2 comparisons reaching BH-corrected significance:")
    sig = vs_df[vs_df.significant_bh]
    print(sig.to_string(index=False) if len(sig) else "        (none)")

    # -- hx5 vs sac_her, full do-no-harm check across all 11 conditions ------
    dnh_rows = []
    for condition in ALL_CONDITIONS:
        row = compare_two_methods(combined, "sac_her_recovery_v4_hx5", "sac_her", condition)
        if row is not None:
            dnh_rows.append(row)
    ...
    print("\n[eval] hx5 vs sac_her, do-no-harm check across all 11 conditions:")
    ...
    # -- hx5 vs v3, goal-spoof conditions ONLY (v3 is n=150, unpaired vs hx5's --
    # n=450 -- reported separately, not pooled into the BH-corrected grid above).
    v3_rows = []
    for condition in GOAL_SPOOF_CONDITIONS:
        row = compare_two_methods(combined, "sac_her_recovery_v4_hx5", "sac_her_recovery_v3", condition)
        if row is not None:
            v3_rows.append(row)
